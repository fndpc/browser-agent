...
- Автономный AI-агент
- Security layer
- Управление контекстом(Создает сжатый snapshot(интерактивные элементы + важный текст) и использует его)
- Sub-agent architecture(BrowserAgent, NavigationAgent, DOMAgent)
- Агент адаптируется к неудачам / выбирает иную стратегию
## Установка
...
Если нужен только runtime (без dev-инструментов), можно поставить:
...
Положите эти значения в `.env` в корне проекта (файл будет автоматически подхвачен при запуске).

```sh
OPENAI_API_KEY="..."
OPENAI_MODEL="gpt-5.1-2025-11-13"  # или аналог, который доступен в вашем аккаунте
# Необязательно. Если не установлен то используется стандартный эндпоинт
# OPENAI_BASE_URL="https://api.openai.com/v1"
#
# Также можно использовать кастомный gateway (Совместимый с OpenAI sdk):
# OPENAI_BASE_URL="https://ru-2.gateway.nekocode.app/alpha/v1"
```
...
- `--slowmo-ms <int>` — задержка Playwright (в мс) между действиями.
